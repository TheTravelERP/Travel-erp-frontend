// src/features/inventory/hotel/components/HotelTable.tsx
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "hotel_code", label: t("hotel.colCode"), sortable: true, minWidth: 100 },
    { id: "hotel_name", label: t("hotel.colName"), sortable: true, minWidth: 200 },
    { id: "hotel_group", label: t("hotel.colGroup"), minWidth: 130 },
    { id: "star_rating", label: t("hotel.colStarRating"), sortable: true, align: "center", minWidth: 90 },
    { id: "city", label: t("hotel.colCity"), sortable: true, minWidth: 120 },
    { id: "phone", label: t("hotel.colPhone"), minWidth: 130 },
...
export default function HotelTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash ? await bulkRestoreHotels(uuids) : await bulkDeleteHotels(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreHotelByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteHotelByUuid(actionUuid);
              ...
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    />
                    <Typography fontWeight={600}>{row.hotel_name}</Typography>
                  </Stack>
                  {renderActiveChip(row.is_active)}
                </Stack>

                <Typography variant="caption">
                  {row.hotel_code} &bull; {row.city}
                </Typography>
            ...
            {!loading &&
              rows.map((row) => (
                <TableRow key={row.uuid} hover selected={selected.has(row.uuid)}>
                  ...
                  <TableCell>{row.hotel_code}</TableCell>
                  <TableCell>{row.hotel_name}</TableCell>
                  <TableCell>{row.hotel_group}</TableCell>
                  <TableCell align="center">{row.star_rating ?? "-"}</TableCell>
