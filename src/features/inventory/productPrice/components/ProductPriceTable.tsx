// src/features/inventory/productPrice/components/ProductPriceTable.tsx
...
import type { ProductPriceListItem } from "../productPrice.types";
import ConfirmDialog from "../../../../components/common/ConfirmDialog";
import SortableTableCell from "../../../../components/common/SortableTableCell";
import {
  bulkDeleteProductPrices,
  bulkRestoreProductPrices,
  deleteProductPriceByUuid,
  restoreProductPriceByUuid,
} from "../productPrice.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: ProductPriceListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof ProductPriceListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "product_name", label: t("productPrice.colProduct"), minWidth: 180 },
    { id: "price_code", label: t("productPrice.colPriceCode"), sortable: true, minWidth: 110 },
    { id: "valid_from", label: t("productPrice.colValidFrom"), sortable: true, minWidth: 110 },
    { id: "valid_to", label: t("productPrice.colValidTo"), sortable: true, minWidth: 110 },
    { id: "cost_price", label: t("productPrice.colCostPrice"), sortable: true, align: "right", minWidth: 110 },
    { id: "sell_price", label: t("productPrice.colSellPrice"), sortable: true, align: "right", minWidth: 110 },
    { id: "currency_code", label: t("productPrice.colCurrency"), minWidth: 90 },
  ];
}
...
export default function ProductPriceTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreProductPrices(uuids)
        : await bulkDeleteProductPrices(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreProductPriceByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteProductPriceByUuid(actionUuid);
  ...
  const rowActions = (row: ProductPriceListItem) => (
    <Stack direction="row" spacing={0.5} justifyContent="flex-end">
      <IconButton
        ...
        onClick={() =>
          navigate(
            isTrash
              ? `/app/inventory/product-price-list/${row.uuid}?is_deleted=true`
              : `/app/inventory/product-price-list/${row.uuid}`,
          )
        }
      >
      ...
      {!isTrash && (
        <>
          <IconButton size="small" onClick={() => navigate(`/app/inventory/product-price-list/${row.uuid}/edit`)}>
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  ...
                  <Chip size="small" label={row.price_code} />
                </Stack>

                <Typography variant="caption">
                  {row.valid_from} &ndash; {row.valid_to} &bull; {row.currency_code} {row.sell_price}
                </Typography>
            ...
            {!loading &&
              rows.map((row) => (
                <TableRow key={row.uuid} hover selected={selected.has(row.uuid)}>
                  ...
                  <TableCell>{row.price_code}</TableCell>
