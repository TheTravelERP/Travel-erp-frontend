// src/features/inventory/insuranceProvider/components/InsuranceProviderTable.tsx
...
import type { InsuranceProviderListItem } from "../insuranceProvider.types";
import ConfirmDialog from "../../../../components/common/ConfirmDialog";
import SortableTableCell from "../../../../components/common/SortableTableCell";
import {
  bulkDeleteInsuranceProviders,
  bulkRestoreInsuranceProviders,
  deleteInsuranceProviderByUuid,
  restoreInsuranceProviderByUuid,
} from "../insuranceProvider.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: InsuranceProviderListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof InsuranceProviderListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "provider_name", label: t("insuranceProvider.colName"), sortable: true, minWidth: 200 },
    { id: "contact_person", label: t("insuranceProvider.colContactPerson"), minWidth: 150 },
    { id: "phone", label: t("insuranceProvider.colPhone"), minWidth: 130 },
    { id: "email", label: t("insuranceProvider.colEmail"), minWidth: 180 },
...
export default function InsuranceProviderTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreInsuranceProviders(uuids)
        : await bulkDeleteInsuranceProviders(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreInsuranceProviderByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteInsuranceProviderByUuid(actionUuid);
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.contact_person} &bull; {row.phone}
                </Typography>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/inventory/insurance/${row.uuid}/edit`)}>
