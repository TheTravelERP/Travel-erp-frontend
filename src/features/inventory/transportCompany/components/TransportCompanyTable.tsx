// src/features/inventory/transportCompany/components/TransportCompanyTable.tsx
...
import type { TransportCompanyListItem } from "../transportCompany.types";
import ConfirmDialog from "../../../../components/common/ConfirmDialog";
import SortableTableCell from "../../../../components/common/SortableTableCell";
import {
  bulkDeleteTransportCompanies,
  bulkRestoreTransportCompanies,
  deleteTransportCompanyByUuid,
  restoreTransportCompanyByUuid,
} from "../transportCompany.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: TransportCompanyListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof TransportCompanyListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "company_name", label: t("transportCompany.colName"), sortable: true, minWidth: 200 },
    { id: "contact_person", label: t("transportCompany.colContactPerson"), minWidth: 150 },
    { id: "mobile", label: t("transportCompany.colMobile"), minWidth: 130 },
    { id: "city", label: t("transportCompany.colCity"), minWidth: 120 },
    { id: "is_active", label: t("common.status"), sortable: true, minWidth: 100 },
  ];
}

export default function TransportCompanyTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreTransportCompanies(uuids)
        : await bulkDeleteTransportCompanies(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreTransportCompanyByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteTransportCompanyByUuid(actionUuid);
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.contact_person} &bull; {row.city}
                </Typography>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/inventory/vendors/${row.uuid}/edit`)}>
