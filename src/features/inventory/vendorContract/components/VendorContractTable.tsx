// src/features/inventory/vendorContract/components/VendorContractTable.tsx
...
import {
  ...
  deleteVendorContractByUuid,
  restoreVendorContractByUuid,
} from "../vendorContract.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: VendorContractListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof VendorContractListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "contract_code", label: t("vendorContract.colCode"), sortable: true, minWidth: 130 },
    { id: "contract_name", label: t("vendorContract.colName"), sortable: true, minWidth: 200 },
    { id: "vendor_name", label: t("vendorContract.colVendor"), minWidth: 160 },
    { id: "contract_type", label: t("vendorContract.colType"), minWidth: 120 },
    { id: "valid_from", label: t("vendorContract.colValidFrom"), sortable: true, minWidth: 110 },
    { id: "valid_to", label: t("vendorContract.colValidTo"), sortable: true, minWidth: 110 },
    { id: "status", label: t("vendorContract.colStatus"), sortable: true, minWidth: 100 },
  ];
}

export default function VendorContractTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreVendorContracts(uuids)
        : await bulkDeleteVendorContracts(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreVendorContractByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteVendorContractByUuid(actionUuid);
  ...
  const rowActions = (row: VendorContractListItem) => (
    <Stack direction="row" spacing={0.5} justifyContent="flex-end">
      <IconButton
        ...
        onClick={() =>
          navigate(
            isTrash
              ? `/app/inventory/contracts/${row.uuid}?is_deleted=true`
              : `/app/inventory/contracts/${row.uuid}`,
          )
        }
      >
      ...
      {!isTrash && (
        <>
          <IconButton size="small" onClick={() => navigate(`/app/inventory/contracts/${row.uuid}/edit`)}>
              ...
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    />
                    <Typography fontWeight={600}>{row.contract_name}</Typography>
                  </Stack>
                  {renderStatusChip(row.status)}
                </Stack>

                <Typography variant="caption">
                  {row.contract_code} &bull; {row.vendor_name} &bull; {row.contract_type}
                </Typography>
