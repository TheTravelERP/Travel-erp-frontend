// src/features/inventory/product/components/ProductTable.tsx
...
interface TableColumn {
  id: keyof ProductListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "product_code", label: t("product.colCode"), sortable: true, minWidth: 120 },
    { id: "product_name", label: t("product.colName"), sortable: true, minWidth: 200 },
    { id: "location_name", label: t("product.colLocation"), minWidth: 150 },
    { id: "service_type_name", label: t("product.colServiceType"), minWidth: 140 },
    { id: "vendor_name", label: t("product.colVendor"), minWidth: 160 },
...
export default function ProductTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ...
        : await bulkDeleteProducts(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreProductByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteProductByUuid(actionUuid);
  ...
  const rowActions = (row: ProductListItem) => (
    <Stack direction="row" spacing={0.5} justifyContent="flex-end">
      <IconButton
        ...
        onClick={() =>
          navigate(
            isTrash
              ? `/app/inventory/product-master/${row.uuid}?is_deleted=true`
              : `/app/inventory/product-master/${row.uuid}`,
          )
        }
      >
      ...
      {!isTrash && (
        <>
          <IconButton size="small" onClick={() => navigate(`/app/inventory/product-master/${row.uuid}/edit`)}>
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.product_code} &bull; {row.location_name} &bull; {row.service_type_name} &bull; {row.vendor_name}
                </Typography>
