// src/features/inventory/airline/components/AirlineTable.tsx
...
import {
  ...
  bulkRestoreAirlines,
  deleteAirlineByUuid,
  restoreAirlineByUuid,
} from "../airline.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: AirlineListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof AirlineListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "airline_code", label: t("airline.colCode"), sortable: true, minWidth: 100 },
    { id: "airline_name", label: t("airline.colName"), sortable: true, minWidth: 200 },
    { id: "icao_code", label: t("airline.colIcaoCode"), minWidth: 100 },
    { id: "country", label: t("airline.colCountry"), sortable: true, minWidth: 130 },
    { id: "phone", label: t("airline.colPhone"), minWidth: 130 },
...
export default function AirlineTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash ? await bulkRestoreAirlines(uuids) : await bulkDeleteAirlines(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreAirlineByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteAirlineByUuid(actionUuid);
              ...
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    />
                    <Typography fontWeight={600}>{row.airline_name}</Typography>
                  </Stack>
                  {renderActiveChip(row.is_active)}
                </Stack>

                <Typography variant="caption">
                  {row.airline_code} &bull; {row.country}
                </Typography>
            ...
            {!loading &&
              rows.map((row) => (
                <TableRow key={row.uuid} hover selected={selected.has(row.uuid)}>
                  ...
                  <TableCell>{row.airline_code}</TableCell>
