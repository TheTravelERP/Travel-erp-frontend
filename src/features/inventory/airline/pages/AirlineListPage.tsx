// src/features/inventory/airline/pages/AirlineListPage.tsx
...
import AirlineFilters, {
  type AirlineFilterValues,
} from "../components/AirlineFilters";

import { usePermission } from "../../../../hooks/usePermission";
import { useCsvImport } from "../../../../hooks/useCsvImport";
import { getAirlines, importAirlinesFromCsv } from "../airline.api";
import type { AirlineListItem } from "../airline.types";

export default function AirlineListPage() {
  ...
  const appliedFilters: AirlineFilterValues = {
  ...
  const [draftFilters, setDraftFilters] =
    useState<AirlineFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<AirlineListItem[]>([]);
  ...
  const handleExport = (format: "csv" | "excel" | "pdf") => {
    ...
    window.open(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/airlines/export?${params}`,
      "_blank",
    );
  };
  ...
  const {
    ...
    openFilePicker,
    onFileInputChange,
  } = useCsvImport(importAirlinesFromCsv, fetchData);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.inventory.airlines")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.inventory.airlines") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.inventory.airlines"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/inventory/airlines/create"),
              }
        }
        secondaryActions={[
          {
