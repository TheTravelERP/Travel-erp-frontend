// src/features/inventory/ziyarat/components/ZiyaratTable.tsx
...
import {
  ...
  bulkRestoreZiyarats,
  deleteZiyaratByUuid,
  restoreZiyaratByUuid,
} from "../ziyarat.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: ZiyaratListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof ZiyaratListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "name", label: t("ziyarat.colName"), sortable: true, minWidth: 200 },
    { id: "city", label: t("ziyarat.colCity"), minWidth: 120 },
    { id: "duration_hours", label: t("ziyarat.colDuration"), sortable: true, align: "center", minWidth: 100 },
    { id: "default_cost", label: t("ziyarat.colDefaultCost"), sortable: true, align: "right", minWidth: 120 },
    { id: "is_active", label: t("common.status"), sortable: true, minWidth: 100 },
  ];
}

export default function ZiyaratTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash ? await bulkRestoreZiyarats(uuids) : await bulkDeleteZiyarats(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreZiyaratByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteZiyaratByUuid(actionUuid);
