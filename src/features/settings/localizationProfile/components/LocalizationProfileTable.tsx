// src/features/settings/localizationProfile/components/LocalizationProfileTable.tsx
...
import type { LocalizationProfileListItem } from "../localizationProfile.types";
import ConfirmDialog from "../../../../components/common/ConfirmDialog";
import SortableTableCell from "../../../../components/common/SortableTableCell";
import {
  bulkDeleteLocalizationProfiles,
  bulkRestoreLocalizationProfiles,
  deleteLocalizationProfileByUuid,
  restoreLocalizationProfileByUuid,
} from "../localizationProfile.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: LocalizationProfileListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof LocalizationProfileListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "profile_name", label: t("localizationProfile.colName"), sortable: true, minWidth: 160 },
    { id: "country_code", label: t("common.country"), sortable: true, minWidth: 90 },
    { id: "default_currency_code", label: t("localizationProfile.colCurrency"), minWidth: 90 },
    { id: "registration_label", label: t("settings.registrationLabel"), minWidth: 120 },
  ];
}

export default function LocalizationProfileTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreLocalizationProfiles(uuids)
        : await bulkDeleteLocalizationProfiles(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreLocalizationProfileByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteLocalizationProfileByUuid(actionUuid);
              ...
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    />
                    <Typography fontWeight={600}>{row.profile_name}</Typography>
                  </Stack>
                </Stack>

                <Typography variant="caption">
                  {row.country_code} &bull; {row.default_currency_code} &bull; {row.registration_label}
                </Typography>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/localization-profiles/${row.uuid}`)}>
                          <VisibilityIcon fontSize="small" />
                        </IconButton>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/localization-profiles/${row.uuid}/edit`)}>
            ...
            {!loading &&
              rows.map((row) => (
                <TableRow key={row.uuid} hover selected={selected.has(row.uuid)}>
                  ...
                  <TableCell>{row.default_currency_code}</TableCell>
                  <TableCell>{row.registration_label}</TableCell>
                  <TableCell align="right" sx={{ whiteSpace: "nowrap" }}>
