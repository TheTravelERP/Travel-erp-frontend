// src/features/settings/localizationProfile/pages/LocalizationProfileListPage.tsx
...
import LocalizationProfileTable from "../components/LocalizationProfileTable";
import LocalizationProfileFilters, {
  type LocalizationProfileFilterValues,
} from "../components/LocalizationProfileFilters";

import { usePermission } from "../../../../hooks/usePermission";
import { getLocalizationProfiles } from "../localizationProfile.api";
import type { LocalizationProfileListItem } from "../localizationProfile.types";

export default function LocalizationProfileListPage() {
  ...
  const appliedFilters: LocalizationProfileFilterValues = {
  ...
  const [draftFilters, setDraftFilters] =
    useState<LocalizationProfileFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<LocalizationProfileListItem[]>([]);
  ...
  const fetchData = async (signal?: AbortSignal) => {
    try {
      ...
      const res = await getLocalizationProfiles(
        {
          page,
          page_size: pageSize,
          is_deleted: isTrash,
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.settings.localization_profile")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.settings.localization_profile") },
        ]}
        primaryAction={
          isTrash
            ? {
            ...
            : {
                ...
                onClick: () => navigate("/app/settings/localization-profiles/create"),
              }
        }
        secondaryActions={[
          {
