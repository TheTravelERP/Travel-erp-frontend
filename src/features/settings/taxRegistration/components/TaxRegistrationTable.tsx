// src/features/settings/taxRegistration/components/TaxRegistrationTable.tsx
...
import {
  bulkDeleteTaxRegistrations,
  bulkRestoreTaxRegistrations,
  deleteTaxRegistrationByUuid,
  restoreTaxRegistrationByUuid,
} from "../taxRegistration.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: TaxRegistrationListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof TaxRegistrationListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "label", label: t("taxRegistration.colLabel"), sortable: true, minWidth: 120 },
    { id: "registration_number", label: t("taxRegistration.colNumber"), sortable: true, minWidth: 160 },
    { id: "country", label: t("common.country"), minWidth: 120 },
  ];
}

export default function TaxRegistrationTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreTaxRegistrations(uuids)
        : await bulkDeleteTaxRegistrations(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreTaxRegistrationByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteTaxRegistrationByUuid(actionUuid);
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.registration_number} &bull; {row.country}
                </Typography>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/tax-registration/${row.uuid}/edit`)}>
