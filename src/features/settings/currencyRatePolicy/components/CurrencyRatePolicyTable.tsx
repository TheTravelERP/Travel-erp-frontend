// src/features/settings/currencyRatePolicy/components/CurrencyRatePolicyTable.tsx
...
import {
  ...
  deleteCurrencyRatePolicyByUuid,
  restoreCurrencyRatePolicyByUuid,
} from "../currencyRatePolicy.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: CurrencyRatePolicyListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof CurrencyRatePolicyListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "from_currency_code", label: t("exchangeRate.colFrom"), sortable: true, minWidth: 90 },
    { id: "to_currency_code", label: t("exchangeRate.colTo"), sortable: true, minWidth: 90 },
    { id: "rate_source", label: t("currencyRatePolicy.rateSource"), sortable: true, minWidth: 130 },
    { id: "effective_from", label: t("exchangeRate.colEffectiveFrom"), sortable: true, minWidth: 120 },
  ];
}

export default function CurrencyRatePolicyTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreCurrencyRatePolicies(uuids)
        : await bulkDeleteCurrencyRatePolicies(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreCurrencyRatePolicyByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteCurrencyRatePolicyByUuid(actionUuid);
              ...
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    <Typography fontWeight={600}>
                      {row.from_currency_code} &rarr; {row.to_currency_code}
                    </Typography>
                  </Stack>
                  <Chip size="small" label={row.rate_source} />
                </Stack>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/currency-rate-policy/${row.uuid}/edit`)}>
