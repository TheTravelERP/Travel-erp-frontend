// src/features/settings/communicationProviders/components/CommunicationProviderFilters.tsx
...
import type { ProviderCategory } from "../communicationProvider.types";

export interface CommunicationProviderFilterValues {
  search?: string;
...
interface Props {
  value: CommunicationProviderFilterValues;
  onChange: (v: Partial<CommunicationProviderFilterValues>) => void;
  onApply: () => void;
  onReset: () => void;
}

export default function CommunicationProviderFilters({ value, onChange, onApply, onReset }: Props) {
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, md: 3 }}>
          <TextField
            select
            fullWidth
            label={t("communicationProvider.category")}
            value={value.provider_category ?? ""}
            onChange={(e) => onChange({ provider_category: e.target.value as ProviderCategory })}
          >
