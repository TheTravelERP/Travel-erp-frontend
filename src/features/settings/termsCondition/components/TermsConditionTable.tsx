// src/features/settings/termsCondition/components/TermsConditionTable.tsx
...
import type { TermsConditionListItem } from "../termsCondition.types";
import ConfirmDialog from "../../../../components/common/ConfirmDialog";
import SortableTableCell from "../../../../components/common/SortableTableCell";
import { useLocalizationProfile } from "../../../../hooks/useLocalizationProfile";
import { createFormatters } from "../../../../utils/formatters/localization";
import {
  bulkDeleteTermsConditions,
  bulkRestoreTermsConditions,
  deleteTermsConditionByUuid,
  restoreTermsConditionByUuid,
} from "../termsCondition.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: TermsConditionListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof TermsConditionListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "code", label: t("termsConditions.colCode"), sortable: true, minWidth: 110 },
    { id: "title", label: t("termsConditions.colTitle"), sortable: true, minWidth: 220 },
    { id: "document_type_name", label: t("termsConditions.colDocumentType"), minWidth: 160 },
    { id: "is_default", label: t("termsConditions.colDefault"), sortable: true, minWidth: 100 },
    { id: "is_active", label: t("common.status"), minWidth: 100 },
    { id: "created_at", label: t("common.createdOn"), sortable: true, minWidth: 120 },
  ];
}

export default function TermsConditionTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreTermsConditions(uuids)
        : await bulkDeleteTermsConditions(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreTermsConditionByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteTermsConditionByUuid(actionUuid);
  ...
  const renderDefaultChip = (isDefault: boolean) =>
    isDefault ? <Chip size="small" label={t("common.yes")} color="primary" /> : null;
  ...
  const rowActions = (row: TermsConditionListItem) => (
    <Stack direction="row" spacing={0.5} justifyContent="flex-end">
      <IconButton
        ...
        onClick={() =>
          navigate(
            isTrash
              ? `/app/settings/terms-conditions-master/${row.uuid}?is_deleted=true`
              : `/app/settings/terms-conditions-master/${row.uuid}`,
          )
        }
      >
      ...
      {!isTrash && (
        <>
          <IconButton
            size="small"
            onClick={() => navigate(`/app/settings/terms-conditions-master/${row.uuid}/edit`)}
          >
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.code} &bull; {row.document_type_name}
                  {row.is_default ? ` • ${t("termsConditions.isDefault")}` : ""}
                </Typography>
            ...
            {!loading &&
              rows.map((row) => (
                <TableRow key={row.uuid} hover selected={selected.has(row.uuid)}>
                  ...
                  <TableCell>{renderDefaultChip(row.is_default)}</TableCell>
