// src/features/settings/termsCondition/pages/TermsConditionListPage.tsx
...
import TermsConditionTable from "../components/TermsConditionTable";
import TermsConditionFilters, {
  type TermsConditionFilterValues,
} from "../components/TermsConditionFilters";

import { usePermission } from "../../../../hooks/usePermission";
import { useCsvImport } from "../../../../hooks/useCsvImport";
import { getTermsConditions, importTermsConditionsFromCsv } from "../termsCondition.api";
import type { TermsConditionListItem } from "../termsCondition.types";

export default function TermsConditionListPage() {
  ...
  const appliedFilters: TermsConditionFilterValues = {
    search: searchParams.get("search") || "",
    document_type_uuid: searchParams.get("document_type_uuid") || "",
    is_default: searchParams.get("is_default") || "",
    is_active: searchParams.get("is_active") || "",
    from_date: searchParams.get("from_date") || "",
    to_date: searchParams.get("to_date") || "",
  };

  const [draftFilters, setDraftFilters] = useState<TermsConditionFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<TermsConditionListItem[]>([]);
      ...
      const res = await getTermsConditions(
        {
          ...
          ...appliedFilters,
          is_default: appliedFilters.is_default ? appliedFilters.is_default === "true" : undefined,
          is_active: appliedFilters.is_active ? appliedFilters.is_active === "true" : undefined,
        },
        signal,
      );
  ...
  const handleExport = (format: "csv" | "excel" | "pdf") => {
    ...
    window.open(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/terms-conditions/export?${params}`,
      "_blank",
    );
  };
  ...
  const {
    ...
    openFilePicker,
    onFileInputChange,
  } = useCsvImport(importTermsConditionsFromCsv, fetchData);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.settings.terms_conditions_master")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.settings.terms_conditions_master") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.settings.terms_conditions_master"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/settings/terms-conditions-master/create"),
              }
        }
        secondaryActions={[
          {
