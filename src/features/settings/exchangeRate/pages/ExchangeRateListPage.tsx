// src/features/settings/exchangeRate/pages/ExchangeRateListPage.tsx
...
import ExchangeRateFilters, {
  type ExchangeRateFilterValues,
} from "../components/ExchangeRateFilters";

import { usePermission } from "../../../../hooks/usePermission";
import { useCsvImport } from "../../../../hooks/useCsvImport";
import { getExchangeRates, importExchangeRatesFromCsv } from "../exchangeRate.api";
...
export default function ExchangeRateListPage() {
  ...
  const appliedFilters: ExchangeRateFilterValues = {
  ...
  const [draftFilters, setDraftFilters] =
    useState<ExchangeRateFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<ExchangeRateListItem[]>([]);
  ...
  const handleExport = (format: "csv" | "excel" | "pdf") => {
    ...
    window.open(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/exchange-rates/export?${params}`,
      "_blank",
    );
  };
  ...
  const {
    ...
    openFilePicker,
    onFileInputChange,
  } = useCsvImport(importExchangeRatesFromCsv, fetchData);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.settings.exchange_rate_master")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.settings.exchange_rate_master") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.settings.exchange_rate_master"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/settings/exchange-rate-master/create"),
              }
        }
        secondaryActions={[
          {
