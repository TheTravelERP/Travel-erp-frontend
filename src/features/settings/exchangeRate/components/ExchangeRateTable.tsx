// src/features/settings/exchangeRate/components/ExchangeRateTable.tsx
...
import {
  ...
  deleteExchangeRateByUuid,
  restoreExchangeRateByUuid,
} from "../exchangeRate.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: ExchangeRateListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof ExchangeRateListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    ...
    { id: "rate", label: t("exchangeRate.colRate"), sortable: true, minWidth: 110 },
    { id: "effective_from", label: t("exchangeRate.colEffectiveFrom"), sortable: true, minWidth: 120 },
    { id: "effective_to", label: t("exchangeRate.colEffectiveTo"), minWidth: 120 },
  ];
}
...
export default function ExchangeRateTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreExchangeRates(uuids)
        : await bulkDeleteExchangeRates(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreExchangeRateByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteExchangeRateByUuid(actionUuid);
              ...
              <CardContent>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/exchange-rate-master/${row.uuid}/edit`)}>
