// src/features/settings/documentType/pages/DocumentTypeListPage.tsx
...
import DocumentTypeFilters, {
  type DocumentTypeFilterValues,
} from "../components/DocumentTypeFilters";

import { usePermission } from "../../../../hooks/usePermission";
import { useCsvImport } from "../../../../hooks/useCsvImport";
import { getDocumentTypes, importDocumentTypesFromCsv } from "../documentType.api";
...
export default function DocumentTypeListPage() {
  ...
  const appliedFilters: DocumentTypeFilterValues = {
    search: searchParams.get("search") || "",
    module_name: searchParams.get("module_name") || "",
    is_system: searchParams.get("is_system") || "",
  ...
  const [draftFilters, setDraftFilters] =
    useState<DocumentTypeFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<DocumentTypeListItem[]>([]);
  ...
  const handleExport = (format: "csv" | "excel" | "pdf") => {
    ...
    window.open(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/document-types/export?${params}`,
      "_blank",
    );
  };
  ...
  const {
    ...
    openFilePicker,
    onFileInputChange,
  } = useCsvImport(importDocumentTypesFromCsv, fetchData);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.settings.document_type_master")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.settings.document_type_master") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.settings.document_type_master"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/settings/document-type-master/create"),
              }
        }
        secondaryActions={[
          {
