// src/features/settings/currencyMaster/components/CurrencyMasterTable.tsx
...
import {
  bulkDeleteCurrencyMasters,
  bulkRestoreCurrencyMasters,
  deleteCurrencyMasterByUuid,
  restoreCurrencyMasterByUuid,
} from "../currencyMaster.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: CurrencyMasterListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof CurrencyMasterListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "code", label: t("currencyMaster.colCode"), sortable: true, minWidth: 90 },
    { id: "name", label: t("currencyMaster.colName"), sortable: true, minWidth: 200 },
    { id: "symbol", label: t("currencyMaster.colSymbol"), minWidth: 90 },
    { id: "decimal_places", label: t("currencyMaster.colDecimalPlaces"), sortable: true, minWidth: 90 },
  ];
}

export default function CurrencyMasterTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreCurrencyMasters(uuids)
        : await bulkDeleteCurrencyMasters(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreCurrencyMasterByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteCurrencyMasterByUuid(actionUuid);
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.code} &bull; {row.symbol} &bull; {row.decimal_places} dp
                </Typography>
                ...
                <Stack direction="row" justifyContent="flex-end" alignItems="center">
                  <Stack direction="row" spacing={0.5}>
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/currency-master/${row.uuid}/edit`)}>
