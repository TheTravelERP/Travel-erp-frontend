// src/features/settings/currencyMaster/pages/CurrencyMasterListPage.tsx
...
import CurrencyMasterTable from "../components/CurrencyMasterTable";
import CurrencyMasterFilters, {
  type CurrencyMasterFilterValues,
} from "../components/CurrencyMasterFilters";

import { usePermission } from "../../../../hooks/usePermission";
import { useCsvImport } from "../../../../hooks/useCsvImport";
import { getCurrencyMasters, importCurrencyMastersFromCsv } from "../currencyMaster.api";
import type { CurrencyMasterListItem } from "../currencyMaster.types";

export default function CurrencyMasterListPage() {
  ...
  const appliedFilters: CurrencyMasterFilterValues = {
  ...
  const [draftFilters, setDraftFilters] =
    useState<CurrencyMasterFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<CurrencyMasterListItem[]>([]);
  ...
  const fetchData = async (signal?: AbortSignal) => {
    try {
      ...
      const res = await getCurrencyMasters(
        {
          page,
          page_size: pageSize,
          is_deleted: isTrash,
  ...
  const handleExport = (format: "csv" | "excel" | "pdf") => {
    ...
    window.open(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/currency-masters/export?${params}`,
      "_blank",
    );
  };
  ...
  const {
    ...
    openFilePicker,
    onFileInputChange,
  } = useCsvImport(importCurrencyMastersFromCsv, fetchData);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.settings.currency_master")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.settings.currency_master") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.settings.currency_master"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/settings/currency-master/create"),
              }
        }
        secondaryActions={[
          {
