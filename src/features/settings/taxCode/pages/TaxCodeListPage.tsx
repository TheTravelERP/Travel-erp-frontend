// src/features/settings/taxCode/pages/TaxCodeListPage.tsx
...
import { getTaxCodes } from "../taxCode.api";
import type { TaxCodeListItem } from "../taxCode.types";

export default function TaxCodeListPage() {
  ...
  const [rows, setRows] = useState<TaxCodeListItem[]>([]);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.settings.tax_code_master")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.settings.tax_code_master") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.settings.tax_code_master"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/settings/tax-code-master/create"),
              }
        }
        overflowActions={[
          {
