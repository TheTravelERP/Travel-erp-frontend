// src/features/settings/taxCode/components/TaxCodeTable.tsx
...
import type { TaxCodeListItem } from "../taxCode.types";
...
import {
  ...
  bulkRestoreTaxCodes,
  deleteTaxCodeByUuid,
  restoreTaxCodeByUuid,
} from "../taxCode.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";
import { getErrorMessage } from "../../../../utils/errorMessage";

interface Props {
  rows: TaxCodeListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof TaxCodeListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "code", label: t("taxCode.colCode"), sortable: true, minWidth: 100 },
    { id: "name", label: t("taxCode.colName"), sortable: true, minWidth: 200 },
    { id: "rate", label: t("taxCode.colRate"), sortable: true, align: "right", minWidth: 100 },
    { id: "tax_type", label: t("taxCode.colTaxType"), minWidth: 150 },
    { id: "is_active", label: t("common.active"), minWidth: 100 },
  ];
}

export default function TaxCodeTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestoreTaxCodes(uuids)
        : await bulkDeleteTaxCodes(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restoreTaxCodeByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deleteTaxCodeByUuid(actionUuid);
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {row.code} &bull; {row.rate}% {row.tax_type ? `— ${row.tax_type}` : ""}
                </Typography>
                  ...
                  <Stack direction="row" spacing={0.5}>
                    <IconButton
                      ...
                      onClick={() =>
                        navigate(
                          isTrash
                            ? `/app/settings/tax-code-master/${row.uuid}?is_deleted=true`
                            : `/app/settings/tax-code-master/${row.uuid}`,
                        )
                      }
                    >
                    ...
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/settings/tax-code-master/${row.uuid}/edit`)}>
