// src/features/package/packageType/components/PackageTypeTable.tsx
...
import {
  ...
  deletePackageTypeByUuid,
  restorePackageTypeByUuid,
} from "../packageType.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";

/* ================= TYPES ================= */

interface Props {
  rows: PackageTypeListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof PackageTypeListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "code", label: t("packageType.colCode"), sortable: true, minWidth: 100 },
    { id: "name", label: t("packageType.colName"), sortable: true, minWidth: 200 },
    { id: "category", label: t("packageType.colCategory"), sortable: true, minWidth: 140 },
    { id: "sort_order", label: t("packageType.colSortOrder"), sortable: true, align: "center", minWidth: 100 },
    { id: "is_active", label: t("packageType.colStatus"), sortable: true, minWidth: 100 },
    { id: "created_at", label: t("common.createdOn"), sortable: true, minWidth: 140 },
  ];
}

/* ================= COMPONENT ================= */

export default function PackageTypeTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestorePackageTypes(uuids)
        : await bulkDeletePackageTypes(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restorePackageTypeByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deletePackageTypeByUuid(actionUuid);
          ...
          rows.map((row) => (
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">{row.code} &bull; {row.category}</Typography>

                <Divider sx={{ my: 1 }} />

                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Typography variant="caption">{t("packageType.sortOrder")}: {row.sort_order}</Typography>
                  <Stack direction="row" spacing={0.5}>
                    <IconButton
                      ...
                      onClick={() =>
                        navigate(
                          isTrash
                            ? `/app/packages/types/${row.uuid}?is_deleted=true`
                            : `/app/packages/types/${row.uuid}`,
                        )
                      }
                    >
                    ...
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/packages/types/${row.uuid}/edit`)}>
