// src/features/package/packageDetail/components/PackageDetailTable.tsx
...
import {
  ...
  deletePackageDetailByUuid,
  restorePackageDetailByUuid,
} from "../packageDetail.api";
import { useSnackbar } from "../../../../components/ui/SnackbarProvider";

/* ================= TYPES ================= */

interface Props {
  rows: PackageDetailListItem[];
  loading: boolean;
  page: number;
  pageSize: number;
  total: number;
...
interface TableColumn {
  id: keyof PackageDetailListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "title", label: t("packageDetail.colTitle"), sortable: true, minWidth: 260 },
    { id: "is_active", label: t("packageDetail.colStatus"), sortable: true, minWidth: 100 },
...
export default function PackageDetailTable({
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestorePackageDetails(uuids)
        : await bulkDeletePackageDetails(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restorePackageDetailByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deletePackageDetailByUuid(actionUuid);
  ...
  if (isMobile) {
    return (
      <Box>
        ...
        {loading ? (
          [...Array(3)].map((_, i) => <Skeleton key={i} height={90} sx={{ mb: 2 }} />)
        ) : rows.length ? (
          rows.map((row) => (
                  ...
                  <Stack direction="row" spacing={0.5}>
                    <IconButton
                      ...
                      onClick={() =>
                        navigate(
                          isTrash
                            ? `/app/packages/details/${row.uuid}?is_deleted=true`
                            : `/app/packages/details/${row.uuid}`,
                        )
                      }
                    >
                    ...
                    {!isTrash && (
                      <>
                        <IconButton size="small" onClick={() => navigate(`/app/packages/details/${row.uuid}/edit`)}>
