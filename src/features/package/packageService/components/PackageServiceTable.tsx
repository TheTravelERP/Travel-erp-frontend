// src/features/package/packageService/components/PackageServiceTable.tsx
...
import {
  ...
  deletePackageServiceByUuid,
  restorePackageServiceByUuid,
} from "../packageService.api";
...
interface TableColumn {
  id: keyof PackageServiceListItem | string;
  label: string;
  sortable?: boolean;
...
function getColumns(t: TFunction): TableColumn[] {
  return [
    { id: "package_name", label: t("packageService.colPackage"), minWidth: 180 },
    { id: "day_no", label: t("packageService.colDay"), sortable: true, align: "center", minWidth: 80 },
    { id: "service_type", label: t("packageService.colServiceType"), sortable: true, minWidth: 120 },
    { id: "description", label: t("packageService.colDescription"), minWidth: 200 },
    { id: "cost_price", label: t("packageService.colCostPrice"), sortable: true, align: "right", minWidth: 120 },
    { id: "selling_price", label: t("packageService.colSellingPrice"), sortable: true, align: "right", minWidth: 120 },
...
export default function PackageServiceTable({
  rows,
  loading,
  page,
  pageSize,
  ...
  async function handleBulkConfirm() {
    ...
    try {
      ...
      const result = isTrash
        ? await bulkRestorePackageServices(uuids)
        : await bulkDeletePackageServices(uuids);
  ...
  async function handleConfirmAction() {
    ...
    try {
      ...
      if (isTrash) {
        await restorePackageServiceByUuid(actionUuid);
        showSnackbar({ message: t("common.restoredSuccess"), severity: "success" });
      } else {
        await deletePackageServiceByUuid(actionUuid);
            ...
            <Paper key={row.uuid} sx={{ mb: 1 }}>
              <CardContent>
                ...
                <Typography variant="caption">
                  {t("packageService.colDay")} {row.day_no} &bull; {row.service_type}
                </Typography>
            ...
            {!loading &&
              rows.map((row) => (
                <TableRow key={row.uuid} hover selected={selected.has(row.uuid)}>
                  ...
                  <TableCell align="center">{row.day_no}</TableCell>
                  <TableCell>{row.service_type}</TableCell>
                  <TableCell>{row.description}</TableCell>
                  <TableCell align="right">{formatPrice(row.cost_price)}</TableCell>
                  <TableCell align="right">{formatPrice(row.selling_price)}</TableCell>
