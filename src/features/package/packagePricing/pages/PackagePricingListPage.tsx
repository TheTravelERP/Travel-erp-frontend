...
export default function PackagePricingListPage() {
  ...
  const fetchData = async (signal?: AbortSignal) => {
    try {
      ...
      const res = await getPackagePricings(
        {
          page,
          page_size: pageSize,
          is_deleted: isTrash,
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.packages.pricing")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.packages.pricing") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.packages.pricing"),
                icon: <ListAltIcon />,
                variant: "contained",
