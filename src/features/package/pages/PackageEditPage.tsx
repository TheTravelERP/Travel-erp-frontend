// src/features/package/pages/PackageEditPage.tsx
...
import { getPackageByUuid, updatePackageByUuid } from "../package.api";
...
export default function PackageEditPage() {
  ...
  const [defaultValues, setDefaultValues] = useState<PackageFormInput>();
  ...
  async function loadPackage() {
    try {
      const data = await getPackageByUuid(uuid!);
  ...
  async function handleUpdate(data: PackageFormInput) {
    try {
      await updatePackageByUuid(uuid!, { ...data, version_no: versionNo! });
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      ...
      <Paper sx={{ p: 3 }}>
        <PackageForm defaultValues={defaultValues} onSubmit={handleUpdate} />
      </Paper>
    </Box>
  );
}
