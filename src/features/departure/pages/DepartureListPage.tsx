// src/features/departure/pages/DepartureListPage.tsx
...
import DepartureFilters, {
  type DepartureFilterValues,
} from "../components/DepartureFilters";

import { usePermission } from "../../../hooks/usePermission";
import { useCsvImport } from "../../../hooks/useCsvImport";
import { getDepartures, importDeparturesFromCsv } from "../departure.api";
...
export default function DepartureListPage() {
  ...
  const appliedFilters: DepartureFilterValues = {
    search: searchParams.get("search") || "",
    status: searchParams.get("status") || "",
    pkg_uuid: searchParams.get("pkg_uuid") || "",
    from_date: searchParams.get("from_date") || "",
    to_date: searchParams.get("to_date") || "",
  };

  const [draftFilters, setDraftFilters] = useState<DepartureFilterValues>(appliedFilters);
  ...
  const [rows, setRows] = useState<DepartureListItem[]>([]);
  ...
  const handleExport = (format: "csv" | "excel" | "pdf") => {
    ...
    window.open(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/departures/export?${params}`,
      "_blank",
    );
  };
  ...
  const {
    ...
    openFilePicker,
    onFileInputChange,
  } = useCsvImport(importDeparturesFromCsv, fetchData);
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 1 } }}>
      <ListPageToolbar
        title={isTrash ? t("common.trash") : t("menu.packages.departures")}
        breadcrumbs={[
          { label: t("menu.dashboard"), href: "/app/dashboard" },
          { label: isTrash ? t("common.trash") : t("menu.packages.departures") },
        ]}
        primaryAction={
          isTrash
            ? {
                key: "view",
                label: t("menu.packages.departures"),
                icon: <ListAltIcon />,
                variant: "contained",
            ...
            : {
                ...
                onClick: () => navigate("/app/packages/departures/create"),
              }
        }
        secondaryActions={[
          {
